class Solution:
    def numIslands(self, grid: List[List[str]]) -> int:
        ...
        if we are at a 1, surrounded by "0s" return True
        if final traversal is a True, add 1 to counter
        """

        # implementation
        """
        visit each node from left to right, up to down
        at each node, run dfs
        if node is a 0, continue
        if node is a 1:
            - check if it is part of the visited islands hash set
            - if it is, continue
            - if it is not, run dfs to find that island
        continue until r == ROW - 1 and c == COL - 1
        Time complexity: (r * c)^2
        """
